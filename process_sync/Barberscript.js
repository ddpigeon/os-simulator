    ...
    setTimeout(async () => {
        if (waitting > 0) {
            for (let i = 0; i < waitting; i++) {
                ...
                await sleep(2000);
                if(levqueue.size()>0){
                    const customer2 = levqueue.dequeue();
                    queue.enqueue(customer2);
                    console.log(    `Customer Entered the Waiting Room ${customer2} at ${new Date().toLocaleTimeString()}`);
                    waitingRoom.innerHTML += `<div class="bg-blue-500 text-white py-2 px-4 rounded">Customer ${customer2} Entered the Waiting Room at ${new Date().toLocaleTimeString()}</div>`;
                    waitting++;
                    leaveWaitingRoom.removeChild(leaveWaitingRoom.childNodes[0]);
                }
            }
        }
